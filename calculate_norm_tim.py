...
#We need to define the files we wish to compute errors between
#The first file is one for the averaged casev (file0)
#The second is for the standard solution for that setup (file1)
...
parser.add_argument('--output_filename', type=str, default='error')
...
#Determine the hours that we can specify errors at:
...
error_record = open(output_filename+'.txt','w')
error_record.write('eta_norm')
error_record.write(str(eta_norm))
error_record.write('\n')
error_record.write('unorm_Hdiv')
error_record.write(str(unorm_Hdiv))
error_record.write('\n')
error_record.write('unorm_L2')
error_record.write(str(u_norm_L2))
