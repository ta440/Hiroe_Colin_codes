...
if args.pickup:
    ...
    #Initialise the modulation variable fields as the same as the standard variable:
    un_modvar = Function(V1, name="Velocity").project(u_expr)
    etan_modvar = Function(V2, name="Elevation").interpolate(eta_expr)
...
cheby_modvar = cheby_exp(OperatorSolver, operator_in, operator_out,
                  ncheb, tol=1.0e-8, L=L)
...
U_modvar = Function(W)
U_modvar_un, U_modvar_etan = U_modvar.split()

#Modulation variable is the same as the standard at the initial time:
U_modvar_un.assign(un)
U_modvar_etan.assign(etan)
...
if rank==0:
    ...
    field_b = Function(
        ...
        val=b.topological)
    if not args.pickup:
        file_sw.write(field_un, field_etan, field_un_modvar, field_etan_modvar, field_PV, field_b)
...
while t < tmax - 0.5*dt:
    ...
    if rank == 0:
        ...
        if tdump > dumpt - dt*0.5:
            ...
            #Compute the modulation variable at this time step:
            print('Computing the modvar')
            tic = time.time()
            
            V_temp.assign(U) 
            for tc in np.arange(t_count):
               cheby_modvar.apply(V_temp,U_modvar,-dt)
               V_temp.assign(U_modvar)
            
            toc = time.time()
            
            print('Modvar computation took', tic-toc)
